...
import com.sck.util.DataTransform;
...
public class CorpService
{
	...
	{
		int r = 0;
		if(param.getCorpSeq() != null && param.getCorpSeq() > 0)
		{
	...
	{
		int r = 0;
		if(param.getEmpSeq() != null && param.getEmpSeq() > 0)
		{
	...
	{
		int r = 0;
		if(param.getFinnSeq() != null && param.getFinnSeq() > 0)
		{
	...
	{
		int r = 0;
		if(param.getHelpSeq() != null && param.getHelpSeq() > 0)
		{
	...
	public List<Map> findKpiScoreByCorp(Integer corpSeq, String fromDate, String toDate)
	{
		List<Map> list = repository.findKpiScoreByCorp(corpSeq, fromDate, toDate);
		DataTransform transform = new DataTransform();
		list = transform.toPivot(list, "DEPTH_1", "DEPTH_1_NM", "EVAL_YEAR", "SCORE",new String[]{"CORP_SEQ"});
		return list; 
